...
import { SignUpData, TOKEN_NAME, JwtPayload, UserDetailsResponse, BaseResponseMessage } from 'types/auth';
...
const MyAccount = () => {
  ...
  useEffect(()=>{
    ...
    .then(()=>{
      ...
      getUserDetails(authToken).then((res)=>{
        const resData: UserDetailsResponse = res.data;

        // Set Formik Values
        formik.setFieldValue('firstName',resData.data.userDetails.firstName);
        formik.setFieldValue('lastName',resData.data.userDetails.lastName);
        formik.setFieldValue('email',resData.data.userDetails.email);
      })
    })
    .catch((err)=>{
    ...
    (authenticated && 
        (<div className='my-account'>
            <div className='card'>
                ...
                {success && (<span style={sucessMessageStyle}>Succesfully Updated your Profile</span>)}
                ...
                <div style={{display:"flex", justifyContent:"space-between"}}>
                    <p style={{marginBottom:"0px", fontSize: "14px", color: "var(--black-color)"}}>Password</p>
                    <p style={{marginBottom:"0px", color:"#0E2D52", fontWeight:"400", fontSize:"12px"}}>Reset Password</p>
                </div>
                <Input
