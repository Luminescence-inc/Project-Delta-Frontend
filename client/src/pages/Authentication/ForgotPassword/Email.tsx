...
import { LogInData, LogInResponse, TOKEN_NAME } from 'types/auth';
...
const Email = () => {
    ...
    const onSubmit = async (values: LogInData)=>{
        ...
        try {
            const res = await generateVerificationEmail(values.email)
            .catch((err)=>{
  ...
  return (
    <>
     ...
     {sentEmail && (
        <div className='login'>
        <div className='card'>
          <h4 onClick={()=>navigate('/')}>Password Reset Link sent to Email</h4>
        </div>
      </div>
     )}
    </>
